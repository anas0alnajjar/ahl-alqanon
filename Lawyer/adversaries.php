...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
    ...
    $sql = "SELECT adversaries.*, offices.office_name 
            FROM adversaries 
            INNER JOIN offices ON adversaries.office_id = offices.office_id 
            WHERE adversaries.lawyer_id = :lawyer_id";
    ...
    if (!empty($search)) {
        $count_sql .= " AND (fname LIKE :search 
                            OR lname LIKE :search 
                            OR CONCAT(fname, ' ', lname) LIKE :search 
                            OR offices.office_name LIKE :search)";
    }
    ...
    $total_records_query->bindValue(':lawyer_id', $user_id, PDO::PARAM_INT);

    if (!empty($search)) {
        $total_records_query->bindValue(':search', $searchParam, PDO::PARAM_STR);
    }
