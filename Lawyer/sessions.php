...
function getCases($conn, $lawyer_id) {
    $sql = "SELECT DISTINCT c.case_id, c.case_title 
            FROM cases c
            LEFT JOIN sessions s ON c.case_id = s.case_id
            WHERE c.lawyer_id = :lawyer_id OR s.assistant_lawyer = :lawyer_id";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':lawyer_id', $lawyer_id, PDO::PARAM_INT);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
    ...
    $stmt->bindValue(':lawyer_id', $user_id, PDO::PARAM_INT); 
    ...
    $total_records_stmt->bindValue(':lawyer_id', $user_id, PDO::PARAM_INT);
