...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
    ...
    // قاعدة الاستعلام لحساب إجمالي السجلات مع شرط البحث للأدوار الخاصة بالمحامي
    $count_sql = "SELECT COUNT(*) 
                  FROM powers p 
                  WHERE FIND_IN_SET(:user_id, p.lawyer_id)";
    $params = [':user_id' => $user_id];

    if (!empty($search)) {
        $count_sql .= " AND p.role LIKE :search";
    ...
    try {
        ...
        // استعلام لجلب البيانات مع الترحيل وشرط البحث للأدوار الخاصة بالمحامي
        $sql = "SELECT p.power_id, p.role, 
                       (SELECT COUNT(*) FROM admin WHERE admin.role_id = p.power_id) +
                       (SELECT COUNT(*) FROM lawyer WHERE lawyer.role_id = p.power_id) +
                       (SELECT COUNT(*) FROM helpers WHERE helpers.role_id = p.power_id) +
                       (SELECT COUNT(*) FROM clients WHERE clients.role_id = p.power_id AND clients.username IS NOT NULL AND clients.username != '') +
                       (SELECT COUNT(*) FROM managers_office WHERE managers_office.role_id = p.power_id) AS user_count
                FROM powers p
                WHERE FIND_IN_SET(:user_id, p.lawyer_id)";

        if (!empty($search)) {
            $sql .= " AND p.role LIKE :search";
        }
        ...
        // الآن يمكنك استخدام $result لعرض النتائج
        // ...
            ...
            <div class="modal-body">
                <form id="roleForm" method="POST">
                    <div class="form-group">
                        ...
                        <input type="hidden"  id="lawyer_id" name="lawyer_id" value="<?=$user_id?>" required>
                    </div>                    
