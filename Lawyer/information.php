...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
    ...
    if ($office_id !== null) {
        // استعلام لاحتساب عدد الموكلين المرتبطين بالمحامي
        $query_clients_count = "SELECT COUNT(*) AS clients_count FROM clients WHERE lawyer_id = :lawyer_id";
        
        $query_helpers_count = "SELECT COUNT(*) AS helpers_count FROM helpers WHERE lawyer_id = :lawyer_id";
        ...
        $query_open_sessions = "SELECT COUNT(*) AS sessions_coming
                                ...
                                WHERE session_date > CURRENT_DATE AND cases.lawyer_id = :lawyer_id";
        ...
        $query_total_cases = "SELECT COUNT(*) AS total_cases_count FROM cases WHERE lawyer_id = :lawyer_id";
        ...
        $query_monthly_costs = "SELECT SUM(e.amount) AS monthly_costs
                                ...
                                WHERE DATE_FORMAT(e.pay_date, '%Y-%m') = :current_month AND c.lawyer_id = :lawyer_id";
        ...
        $query_monthly_payment = "SELECT SUM(p.amount_paid) AS monthly_paid
                                  ...
                                  WHERE p.received  != 1  AND DATE_FORMAT(p.payment_date, '%Y-%m') = :current_month AND c.lawyer_id = :lawyer_id";

        $query_monthly_payment_total = "SELECT SUM(p.amount_paid) AS monthly_paid
                                        FROM payments p
                                        JOIN cases c ON p.case_id = c.case_id
                                        WHERE DATE_FORMAT(p.payment_date, '%Y-%m') = :current_month AND c.lawyer_id = :lawyer_id";

        try {
            // تنفيذ استعلام عدد الموكلين
            $statement_clients_count = $conn->prepare($query_clients_count);
            $statement_clients_count->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
            $statement_clients_count->execute();
            $clients_count_result = $statement_clients_count->fetch(PDO::FETCH_ASSOC);
            
            // تنفيذ استعلام عدد المساعدين
            $statement_helpers_count = $conn->prepare($query_helpers_count);
            $statement_helpers_count->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
            $statement_helpers_count->execute();
            $helpers_count_result = $statement_helpers_count->fetch(PDO::FETCH_ASSOC);

            // تنفيذ استعلام عدد الجلسات القادمة
            $statement_open_sessions = $conn->prepare($query_open_sessions);
            $statement_open_sessions->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
            ...
            $statement_total_sessions->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
            ...
            $statement_total_cases->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
            ...
            $statement_monthly_costs->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
            ...
            $statement_monthly_payment->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
            ...
            $statement_monthly_payment_total->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
