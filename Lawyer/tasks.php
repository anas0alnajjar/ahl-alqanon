...
if (isset($_SESSION['user_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Lawyer') {
        ...
        function getClients($conn, $lawyer_id) {
        ...
        $helperFilter = isset($_GET['helperFilter']) ? $_GET['helperFilter'] : '';
        ...
        $sql = "SELECT todos.*, 
                ...
                WHERE (todos.lawyer_id = :lawyer_id
                       OR todos.case_id IN (
                            SELECT case_id 
                            FROM cases 
                            WHERE lawyer_id = :lawyer_id
        ...
        if (!empty($helperFilter)) {
            $sql .= " AND todos.helper_id = :helperFilter";
        }
        ...
        if (!empty($helperFilter)) {
            $stmt->bindParam(':helperFilter', $helperFilter, PDO::PARAM_INT);
        }
        ...
        if (!empty($helperFilter)) {
            $count_sql .= " AND todos.helper_id = :helperFilter";
        }
        ...
        $count_stmt->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
        if (!empty($search)) {
        ...
        if (!empty($helperFilter)) {
            $count_stmt->bindParam(':helperFilter', $helperFilter, PDO::PARAM_INT);
        }
...
<div class="container mt-5">
                ...
                <div class="card">
                    ...
                    <form action="tasks.php" class="mt-3" method="GET" id="searchForm">
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                <select id="helperFilter" name="helperFilter" class="form-control">
                                    <option value="">كل الإداريين</option>
                                    <?php foreach ($helpers  as $helper): ?>
                                        <option value="<?= htmlspecialchars($helper['id']) ?>" <?= ($helperFilter == $helper['id']) ? 'selected' : '' ?>><?= htmlspecialchars($helper['helper_name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                                ...
                                <div class="todo-item border p-3 mb-2 rounded" style="direction:rtl;text-align:right;">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div>
                                            ...
                                            <?php } ?>
                                            <span class="task-title <?php echo $todo['read_by_lawyer'] ? 'checked' : ''; ?> mb-0">
    ...
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item <?php echo ($page_number <= 1) ? 'disabled' : ''; ?>">
            <a class="page-link" href="?page_number=<?php echo max(1, $page_number - 1); ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&helperFilter=<?php echo urlencode($helperFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">السابق</a>
        </li>

        <!-- First Page -->
        <?php if ($page_number > 3): ?>
            <li class="page-item">
                <a class="page-link" href="?page_number=1&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&helperFilter=<?php echo urlencode($helperFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">1</a>
            </li>
        ...
        for ($i = $start; $i <= $end; $i++): ?>
            <li class="page-item <?php echo ($page_number == $i) ? 'active' : ''; ?>">
                <a class="page-link" href="?page_number=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&helperFilter=<?php echo urlencode($helperFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>"><?php echo $i; ?></a>
            </li>
        <?php endfor; ?>
        ...
        <?php if ($page_number < $total_pages - 2): ?>
            <li class="page-item disabled"><span class="page-link">...</span></li>
            <li class="page-item">
                <a class="page-link" href="?page_number=<?php echo $total_pages; ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&helperFilter=<?php echo urlencode($helperFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>"><?php echo $total_pages; ?></a>
            </li>
        <?php endif; ?>

        <!-- Next Page -->
        <li class="page-item <?php echo ($page_number >= $total_pages) ? 'disabled' : ''; ?>">
            <a class="page-link" href="?page_number=<?php echo min($total_pages, $page_number + 1); ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&helperFilter=<?php echo urlencode($helperFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">التالي</a>
        </li>
    </ul>
</nav>
    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row mb-3">
                    <input type="hidden" name="lawyer_id" id="lawers" value="<?=$user_id?>">
                    <div class="col-md-12">
                        ...
                        <select id="helper_name" class="form-select" name="helper_id">
                            ...
                            <?php
                                ...
                                $stmt_helpers->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
        ...
        <div class="modal-content" style="max-height:80vh;overflow:auto;">
            ...
            <form id="editTaskForm" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        <input type="hidden" name="lawyer_id" id="edit_lawers" value="<?=$user_id?>">
                        <div class="col-md-12">
