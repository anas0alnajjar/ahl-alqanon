...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
    ...
    if ($pages['judicial_circuits']['read'] == 0) {
    ...
    if (!empty($OfficeId)) {
        ...
        $sql = "SELECT departments.*, offices.office_name FROM departments 
                LEFT JOIN offices ON departments.office_id = offices.office_id 
                WHERE departments.office_id IN ($OfficeId) OR departments.public = 1";

        // Prepare statement with placeholders
        if (!empty($search)) {
            $sql .= " AND (departments.type LIKE ? OR offices.office_name LIKE ?)";
            $sql .= " ORDER BY departments.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
            $sqlCount = "SELECT COUNT(*) FROM departments 
                         LEFT JOIN offices ON departments.office_id = offices.office_id 
                         WHERE departments.office_id IN ($OfficeId)
                         AND (departments.type LIKE ? OR offices.office_name LIKE ?)";
            ...
            $sql .= " ORDER BY departments.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
            $stmt = $conn->prepare($sql);
            $total_records = $conn->query("SELECT COUNT(*) FROM departments 
                                           WHERE office_id IN ($OfficeId)")->fetchColumn();
            $total_pages = ceil($total_records / $total_records_per_page);
        }
    ...
    <div class="modal fade" id="add_client_modal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <h5 style="float: right;" class="modal-title" id="editModalLabel">إضافة دائرة</h5>
                    
                </div>
                <div class="modal-body">

                    <form method="post" class="shadow p-3 mt-5 n-table" action="req/departments-add.php" style="direction:rtl;">
                      ...
                      <div class="mb-3">
                        <label for="types" class="form-label">اسم الدائرة </label>
                        <input type="text" 
                              class="form-control"
                              value="" 
                              name="types"
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="editModalLabel">تعديل الدائرة</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="position: absolute;left: 5px;"></button>
      </div>
      <div class="modal-body">
        <form id="editFormDepartment" method="post" action="req/department-edit.php">
          <div class="mb-3">
            <label for="typeInput" class="form-label">الدائرة</label>
            <input type="text" class="form-control" id="typeInput" name="type" required>
          </div>
          <input type="hidden" value="<?=$OfficeId?>" id="officeIdEdit" name="office_id" required>
    ...
    <div class="container mt-5" style="direction: rtl;">
            <div class="btn-group" style="direction:ltr;">
                ...
                <?php if ($pages['judicial_circuits']['add']) : ?>
                <button class="btn btn-dark" id="disability-add"> اضافة دائرة جديدة</button>
                <?php endif; ?>
            </div>
        <form action="departments_types.php" class="mt-3 n-table" method="GET">
...
<table class="table table-responsive mt-3 n-table">
    <thead>
        <tr>
            ...
            <th scope="col" style="text-wrap:nowrap;text-align:center;font-weight:700;"> الدائرة  </th>
            <?php if ($pages['judicial_circuits']['write'] || $pages['judicial_circuits']['delete']) : ?>
                <th scope="col" style="font-weight:700;text-align:center;"> الإجراءات  </th>
            <?php endif; ?>
        </tr>
    </thead>
    <tbody>
    ...
    foreach ($result as $row): ?>
        <tr>
            ...
            <?php if (isset($row['public']) && $row['public'] == 1) : ?>
                <td style="text-wrap:nowrap;text-align:center;">
                    <button class="btn btn-secondary btn-sm m-auto" disabled>عام</button>
                </td>
            <?php else : ?>
                <?php if ($pages['judicial_circuits']['write'] || $pages['judicial_circuits']['delete']) : ?>
                    <td style="text-wrap:nowrap;text-align:center;">
                        <?php if ($pages['judicial_circuits']['write']) : ?>
                            <button data-id="<?=$row['id']?>" data-office-id="<?=$row['office_id']?>" class="btn btn-warning btn-sm m-auto">تعديل</button>
                        <?php endif; ?>
                        <?php if ($pages['judicial_circuits']['delete']) : ?>
                            <a href="req/departments-delete.php?id=<?=$row['id']?>" class="btn btn-danger m-auto btn-sm delete-button" data-id="<?=$row['id']?>">حذف</a>
                        <?php endif; ?>
                    </td>
                <?php endif; ?>
            <?php endif; ?>
...
<nav aria-label="Page navigation example">
    ...
    <?php } else { ?>
        ...
        <div class="alert alert-info d-flex align-items-center m-3" role="alert" style="max-width: 500px;text-align: right;float: right;">
        <button href="" class="btn btn-dark mx-2" id="disability-add">اضافة دائرة جديدة</button>
        <span class="flex-grow-1">لا يوجد أي دائرة مسجلة حتى الآن</span>
