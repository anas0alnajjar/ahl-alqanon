...
if (isset($_POST['helper_name'])      &&
    ...
    if (empty($helper_name)) {
        ...
        $sql = "UPDATE helpers SET helper_name = ?, `phone`=? , role_id=?, passport_helper=?, national_helper=?, office_id=?
                WHERE `id`=?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$helper_name , $phone, $role_id, $passport_helper, $national_helper, $office_id, $helper_id]);
        $sm = "تم التحديث بنجاح!";
