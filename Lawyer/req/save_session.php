...
try {
    // Check if user is logged in as a lawyer
    if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
        ...
        // تحقق مما إذا كانت القضية تخص المحامي الرئيسي
        $sql = "SELECT lawyer_id FROM cases WHERE case_id = :case_id";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':case_id', $case_id, PDO::PARAM_INT);
        $stmt->execute();
        $case = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($case && $case['lawyer_id'] != $lawyer_id) {
            // إذا كانت القضية لا تخص المحامي الرئيسي، أضف المحامي كمساعد
            $assistant_lawyer = $lawyer_id;
        }
        ...
        $stmt->bindParam(':assistant_lawyer', $assistant_lawyer, is_null($assistant_lawyer) ? PDO::PARAM_NULL : PDO::PARAM_INT);
