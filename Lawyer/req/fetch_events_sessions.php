...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
    ...
    if (!empty($search_conditions_sessions)) {
        $sessions_query .= " AND " . implode(' AND ', $search_conditions_sessions);
    }

    if (!empty($search_conditions_events)) {
        $events_query .= " AND " . implode(' AND ', $search_conditions_events);
    }
    ...
    $total_sessions = $conn->query("SELECT COUNT(*) FROM sessions s 
                                    ...
                                    WHERE c.lawyer_id = $user_id")->fetchColumn();
    $total_events = $conn->query("SELECT COUNT(*) FROM events WHERE lawyer_id = $user_id")->fetchColumn();
    ...
    if (empty($sessions)) {
        $output_sessions .= '<div class="alert alert-info text-center م-auto w-50" role="alert">
        ...
        foreach ($sessions as $item) {
            ...
            if ($pages['sessions']['add'] == 0) {
                $output_sessions .= '<a class="btn btn-primary btn-sm edit-button mx-2" data-id="' . $item['sessions_id'] . '">عرض</a>';
            } else {
                $output_sessions .= '<a class="btn btn-primary btn-sm edit-button mx-2" data-id="' . $item['sessions_id'] . '">تعديل</a>';
            }
            if ($pages['sessions']['delete'] == 1) {
                $output_sessions .= '<a class="btn btn-danger btn-sm delete-button" data-id="' . $item['sessions_id'] . '">حذف</a>';
            }
            $output_sessions .= '</div>
                </div>
            </div>';
    ...
    if (empty($events)) {
        ...
        foreach ($events as $item) {
            ...
            $output_events .= '</div>
                    <div class="timeline-footer">';
            if ($pages['events']['add'] == 0) {
                $output_events .= '<a class="btn btn-primary btn-sm edit-event-btn mx-2" data-id="' . $item['event_id'] . '">عرض</a>';
            } else {
                $output_events .= '<a class="btn btn-primary btn-sm edit-event-btn mx-2" data-id="' . $item['event_id'] . '">تعديل</a>';
            }
            if ($pages['events']['delete'] == 1) {
                $output_events .= '<a class="btn btn-danger btn-sm delete-event-btn" data-id="' . $item['event_id'] . '">حذف</a>';
            }
