...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if (!empty($sessionIds)) {
            $updateSessionSql = "UPDATE sessions SET session_number = ?, session_date = ?, session_hour = ?, session_date_hjri = ?, notes=? WHERE sessions_id = ?";
            $updateSessionStmt = $conn->prepare($updateSessionSql);

            for ($i = 0; $i < count($sessionIds); $i++) {
                $updateSessionStmt->execute([$sessionNumbers[$i], $sessionDates[$i], $sessionHours[$i], $sessionDatesHiri[$i], $sessionNotes[$i], $sessionIds[$i]]);
            }
        }

        // إضافة جلسات جديدة
        if (!empty($sessionNumbersNews)) {
            $insertSessionSql = "INSERT INTO sessions (case_id, session_number, session_date, session_date_hjri, session_hour, notes) VALUES (?, ?, ?, ?, ?, ?)";
