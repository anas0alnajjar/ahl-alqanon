...
if (isset($_POST['admin_pass']) &&
    ...
    $data = 'casemanager_id='.$client_id.'#change_password';

    if (empty($admin_pass)) {
		$em  = "Admin password is required";
		header("Location: ../client-edit.php?perror=$em&$data");
		exit;
        ...
        $sql = "UPDATE tabcasemanager SET
                `password` = ?
                WHERE casemanager_id=?";

        $stmt = $conn->prepare($sql);
        $stmt->execute([$new_pass, $client_id]);
        $sm = "The password has been changed successfully!";
        header("Location: ../client-edit.php?psuccess=$sm&$data");
        exit;
	}
