    ...
    if ($_SESSION['role'] == 'Lawyer') {
        ...
        if ($office_id !== null) {
            ...
            $query_for_session = "SELECT 
                                ...
                                WHERE 
                                    c.lawyer_id = :lawyer_id OR s.assistant_lawyer = :assistant_lawyer
                                ORDER BY 
                                    s.session_date DESC";

            $statementSession = $conn->prepare($query_for_session);
            $statementSession->bindParam(':lawyer_id', $user_id, PDO::PARAM_INT);
            ...
            $response = array(
                'status' => false,
                'message' => 'لم يتم العثور على مكتب مرتبط بهذا المسؤول.'
            );
            echo json_encode($response);
        }
