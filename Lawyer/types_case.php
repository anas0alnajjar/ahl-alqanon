...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
    ...
    if (empty($OfficeId)) {
        echo "No office associated with this manager.";
        exit();
    }

    // Base SQL query with INNER JOIN to get office name
    $sql = "SELECT types_of_cases.*, offices.office_name 
            FROM types_of_cases 
            INNER JOIN offices ON types_of_cases.office_id = offices.office_id
            WHERE types_of_cases.office_id = :OfficeId OR types_of_cases.public = 1 ";
    $sqlCount = "SELECT COUNT(*) 
                 FROM types_of_cases 
                 INNER JOIN offices ON types_of_cases.office_id = offices.office_id
                 WHERE types_of_cases.office_id = :OfficeId";

    // Prepare statement with placeholders for search
    if (!empty($search)) {
        $sql .= " AND (type_case LIKE :search OR offices.office_name LIKE :search)";
        $sqlCount .= " AND (type_case LIKE :search OR offices.office_name LIKE :search)";
        $searchParam = "%$search%";
    }

    // Append pagination to SQL query
    $sql .= " ORDER BY types_of_cases.id DESC LIMIT :offset, :total_records_per_page";

    // Prepare and execute count statement
    $stmtCount = $conn->prepare($sqlCount);
    $stmtCount->bindParam(':OfficeId', $OfficeId, PDO::PARAM_INT);
    if (!empty($search)) {
    ...
    // Prepare and execute main query statement
...
<head>
    ...
    <title>Admin - Types Of Cases</title>
    ...
    <div class="modal fade" id="add_client_modal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <h5 style="float: right;" class="modal-title" id="editModalLabel">إضافة نوع قضية</h5>
                    
                </div>
                <div class="modal-body">

                    <form method="post" class="shadow p-3 mt-5 n-table" action="req/types-cases-add.php" style="direction:rtl;">
                      ...
                      <div class="mb-3">
                        <label for="types" class="form-label">نوع القضية </label>
                        <input type="text" 
                              class="form-control"
                              value="" 
                              name="types"
...
<div style="direction:rtl;" class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editFormTypeCases" method="post" action="req/types-cases-edit.php">
          <div class="mb-3">
            <label for="typeInput" class="form-label">النوع</label>
            <input type="text" class="form-control" id="typeInput" name="type" required>
          </div>
          <input type="hidden" class="form-control" id="officeIdEdit" name="office_id" value="<?=$OfficeId?>" required>
    ...
    <div class="btn-group" style="direction:ltr;">
        ...
        <?php if ($pages['case_types']['add']) : ?>
            <button class="btn btn-dark" id="disability-add"> اضافة نوع جديد</button>
        <?php endif; ?>
    </div>    
        <form action="types_case.php" class="mt-3 n-table" method="GET">
...
<table class="table table-responsive mt-3 n-table">
    <thead>
        <tr>
            ...
            <th scope="col" style="text-wrap:nowrap;text-align:center;font-weight:700;"> النوع  </th>
        <?php if ($pages['case_types']['write'] || $pages['case_types']['delete']) : ?>
    ...
    foreach ($result as $row): ?>
        <tr>
            ...
            <?php else : ?>
                <?php if ($pages['case_types']['write'] || $pages['case_types']['delete']) : ?>
                    <td style="text-wrap:nowrap;text-align:center;">
                        <?php if ($pages['case_types']['write']) : ?>
                            <button data-id="<?=$row['id']?>" data-office-id="<?=$row['office_id']?>" class="btn btn-warning btn-sm m-auto">تعديل</button>
                        <?php endif; ?>
                        <?php if ($pages['case_types']['delete']) : ?>
                            <a href="req/types-cases-delete.php?id=<?=$row['id']?>" class="btn btn-danger m-auto btn-sm delete-button" data-id="<?=$row['id']?>">حذف</a>
                        <?php endif; ?>
                    </td>
                <?php endif; ?>
            <?php endif; ?>
...
<nav aria-label="Page navigation example">
    ...
    <?php } else { ?>
        ...
        <span class="flex-grow-1 ">لا يوجد أنواع للقضايا حتى الآن</span>
