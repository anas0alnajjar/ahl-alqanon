...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Lawyer') {
    ...
    if (!empty($OfficeId)) {
        ...
        $sql = "SELECT costs_type.*, offices.office_name 
                ...
                WHERE costs_type.office_id = $OfficeId OR costs_type.public = 1";
        ...
        if (!empty($search)) {
            ...
            $sqlCount = "SELECT COUNT(*) 
                         FROM costs_type 
                         INNER JOIN offices ON costs_type.office_id = offices.office_id 
                         WHERE costs_type.office_id = $OfficeId
            ...
            $sql .= " ORDER BY costs_type.id DESC LIMIT " . (int)$offset . ", " . (int)$total_records_per_page;
            $stmt = $conn->prepare($sql);
            $total_records = $conn->query("SELECT COUNT(*) FROM costs_type WHERE office_id = ($OfficeId)")->fetchColumn();
            $total_pages = ceil($total_records / $total_records_per_page);
        }
