...
if (isset($_SESSION['user_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Admins') {
        ...
        function getClients($conn, $admin_id) {
        ...
        function getLawyers($conn, $admin_id) {
            $sql = "SELECT lawyer_id, lawyer_name 
                    FROM lawyer 
                    WHERE office_id IN (SELECT office_id FROM offices WHERE admin_id = :admin_id)";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':admin_id', $admin_id, PDO::PARAM_INT);
        ...
        $clients = getClients($conn, $admin_id);
        $lawyers = getLawyers($conn, $admin_id);
        ...
        WHERE (todos.client_id IN (
                    SELECT client_id 
                    FROM clients 
                    WHERE office_id IN (SELECT office_id FROM offices WHERE admin_id = :admin_id)
                ) OR todos.lawyer_id IN (
                    SELECT lawyer_id 
                    FROM lawyer 
                    WHERE office_id IN (SELECT office_id FROM offices WHERE admin_id = :admin_id)
                ))";
                                ...
                                <div class="todo-item border p-3 mb-2 rounded" style="direction:rtl;text-align:right;">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div>
                                            ...
                                            <?php } ?>
                                            <span class="task-title <?php echo $todo['read_by_admin'] ? 'checked' : ''; ?> mb-0">
    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select id="lawers" class="form-select" name="lawyer_id" required>
                            ...
                            <?php if (!empty($lawyers)): ?>
                                    <?php foreach ($lawyers as $lawyer): ?>
                                        <option value="<?= htmlspecialchars($lawyer['lawyer_id']) ?>"><?= htmlspecialchars($lawyer['lawyer_name']) ?></option>
                                    <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="helper_name" class="form-select" name="helper_id">
                            ...
                            <?php
                                $sql_helpers = "
                                    SELECT h.id, h.helper_name 
                                    FROM helpers h
                                    INNER JOIN offices o ON h.office_id = o.office_id
                                    WHERE o.admin_id = :admin_id
                                    ORDER BY h.id;
                                ";
                                $stmt_helpers = $conn->prepare($sql_helpers);
                                $stmt_helpers->bindParam(':admin_id', $admin_id, PDO::PARAM_INT);
                                $stmt_helpers->execute();

                                if ($stmt_helpers->rowCount() > 0) {
                                    while ($row_helper = $stmt_helpers->fetch(PDO::FETCH_ASSOC)) {
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label for="client_name" class="form-label">اختر موكل</label>
                        <select id="client_name" class="form-select" name="client_id">
                            <option value="" selected disabled>اختر موكل...</option>
                            <?php foreach ($clients as $client): ?>
                                <option value="<?= htmlspecialchars($client['client_id']) ?>" <?= ($clientFilter == $client['client_id']) ? 'selected' : '' ?>><?= htmlspecialchars($client['first_name'] . ' ' . $client['last_name']) ?></option>
                            <?php endforeach; ?>
                        </select>
        ...
        <div class="modal-content" style="max-height:80vh;overflow:auto;">
            ...
            <form id="editTaskForm" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="edit_client_name" class="form-label">اختر موكل</label>
