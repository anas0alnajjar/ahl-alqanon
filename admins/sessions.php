...
function getCases($conn, $admin_id) {
    $sql = "SELECT case_id, case_title 
            FROM cases 
            WHERE office_id IN (SELECT office_id FROM offices WHERE admin_id = :admin_id)";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':admin_id', $admin_id, PDO::PARAM_INT);
...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admins') {
    ...
    if (!empty($lawyerFilter)) {
        $sql .= " AND (l.lawyer_id = :lawyerFilter OR al.lawyer_id = :lawyerFilter)";
    }

    $sql .= " ORDER BY 
                CASE
                    WHEN s.session_date >= CURRENT_DATE() THEN 0 -- الجلسات المستقبلية
                    ELSE 1 -- الجلسات القديمة
                END,
                s.session_date ASC
    ...
    if (!empty($lawyerFilter)) {
        $stmt->bindValue(':lawyerFilter', $lawyerFilter, PDO::PARAM_INT);
    }
    ...
    // Get total records count using a single query
    ...
    if (!empty($lawyerFilter)) {
        $total_records_sql .= " AND (l.lawyer_id = :lawyerFilter OR al.lawyer_id = :lawyerFilter)";
    }

    // Prepare and bind parameters for total records count query
    $total_records_stmt = $conn->prepare($total_records_sql);
    $total_records_stmt->bindValue(':admin_id', $admin_id, PDO::PARAM_INT);
    ...
    if (!empty($lawyerFilter)) {
        $total_records_stmt->bindValue(':lawyerFilter', $lawyerFilter, PDO::PARAM_INT);
    }
    ...
    <div class="container mt-5">
        ...
        <form action="sessions.php" class="mt-3" method="GET" id="searchForm">
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    <select id="lawyerFilter" class="form-control">
...
<nav aria-label="Page navigation example">
    <ul class="pagination" style="direction: ltr;float:right;">
        ...
        <li class="page-item <?php echo ($page_number <= 1) ? 'disabled' : ''; ?>">
            <a class="page-link" href="?page_number=<?php echo ($page_number - 1); ?>&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&clientFilter=<?= htmlentities($clientFilter) ?>&lawyerFilter=<?= htmlentities($lawyerFilter) ?>">Previous</a>
        </li>

        <!-- First Page -->
        <?php if ($page_number > 3): ?>
            <li class="page-item">
                <a class="page-link" href="?page_number=1&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&clientFilter=<?= htmlentities($clientFilter) ?>&lawyerFilter=<?= htmlentities($lawyerFilter) ?>">1</a>
            </li>
        ...
        <?php for ($i = max(1, $page_number - 2); $i <= min($total_pages, $page_number + 2); $i++): ?>
            <li class="page-item <?php echo ($page_number == $i) ? 'active' : ''; ?>">
                <a class="page-link" href="?page_number=<?php echo $i; ?>&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&clientFilter=<?= htmlentities($clientFilter) ?>&lawyerFilter=<?= htmlentities($lawyerFilter) ?>"><?php echo $i; ?></a>
            </li>
        <?php endfor; ?>
        ...
        <?php if ($page_number < $total_pages - 2): ?>
            ...
            <li class="page-item">
                <a class="page-link" href="?page_number=<?php echo $total_pages; ?>&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&clientFilter=<?= htmlentities($clientFilter) ?>&lawyerFilter=<?= htmlentities($lawyerFilter) ?>"><?php echo $total_pages; ?></a>
            </li>
        <?php endif; ?>

        <!-- Next Page -->
        <li class="page-item <?php echo ($page_number >= $total_pages) ? 'disabled' : ''; ?>">
            <a class="page-link" href="?page_number=<?php echo ($page_number + 1); ?>&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&clientFilter=<?= htmlentities($clientFilter) ?>&lawyerFilter=<?= htmlentities($lawyerFilter) ?>">Next</a>
        </li>
    </ul>
</nav>
        </div>
