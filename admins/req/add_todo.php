...
if (isset($_SESSION['user_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Admins') {
        ...
        function getWhatsAppSettings($conn, $admin_id) {
            $sqlSetting = "SELECT `host_whatsapp`, `token_whatsapp` FROM `setting` WHERE `admin_id` = ?";
        ...
        $settings = getWhatsAppSettings($conn, $_SESSION['user_id']);
        if (!$settings) {
