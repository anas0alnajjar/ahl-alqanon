...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admins') {
    ...
    if (empty($offices)) {
        echo "No offices associated with this admin.";
        exit();
    }

    $office_ids = implode(',', array_map('intval', $offices));
    ...
    $sql = "SELECT types_of_cases.*, offices.office_name 
            ...
            WHERE types_of_cases.office_id IN ($office_ids)";
    ...
    if (!empty($search)) {
        $sql .= " AND (type_case LIKE ? OR offices.office_name LIKE ?)";
        $sqlCount .= " AND (type_case LIKE ? OR offices.office_name LIKE ?)";
        $searchParam = "%$search%";
    }
