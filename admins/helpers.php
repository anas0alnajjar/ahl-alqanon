...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admins') {
    ...
    // Get the offices related to the admin
    ...
    if (!empty($offices)) {
        ...
        // Base SQL query
        $sql = "SELECT * FROM helpers WHERE office_id IN ($office_ids)";
        ...
        // Count total records
        $total_records_sql = "SELECT COUNT(*) FROM helpers WHERE office_id IN ($office_ids)";
        if (!empty($search)) {
                ...
                <form id="helperForm" method="POST">    
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label class="mb-2" for="lawyer_id555">المحامي</label>
                            <select id="lawyer_id555" class="form-select col-md-12" name="lawyer_id555">
                                <option value="">اختر المحامي...</option>
                                    ...
                                    if (!empty($offices)) {
                                        ...
                                        // جلب المحامين المرتبطين بهذه المكاتب
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <option value="" disabled selected>اختر المكتب</option>
                                <?php
                                $admin_id = $_SESSION['user_id']; // تأكد من أن الجلسة قد بدأت وأن admin_id معرف بشكل صحيح
                        ...
                        <div class="form-group col-md-6">
                            ...
                            $sql_default_role = "SELECT power_id FROM powers WHERE default_role_helper = 1 LIMIT 1";
                            $stmt_default_role = $conn->prepare($sql_default_role);
                            $stmt_default_role->execute();
                            if ($stmt_default_role->rowCount() > 0) {
                                $default_role_id = $stmt_default_role->fetchColumn();
                            }

                            // جلب الأدوار المرتبطة بالمكاتب بالإضافة إلى التحقق من الدور الافتراضي
                            $sql_roles = "SELECT `power_id`, `role` FROM powers WHERE office_id IN (:office_ids) OR default_role_helper = 1";
                            $stmt_roles = $conn->prepare($sql_roles);
                            $stmt_roles->bindParam(':office_ids', implode(',', $offices), PDO::PARAM_STR);
                            $stmt_roles->execute();
                            ...
                            if ($stmt_roles->rowCount() > 0) {
                                foreach ($roles as $row) {
                                    $id = $row["power_id"];
                                    $role = $row["role"];
                                    $selected = ($id == $default_role_id) ? "selected" : "";
