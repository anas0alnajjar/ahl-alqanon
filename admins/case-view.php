                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div id="dynamic_cards" class="row">
                                            ...
                                            if (!empty($offices)) {
                                                ...
                                                $sql_lawyers = "SELECT lawyer_id, lawyer_name FROM lawyer WHERE office_id IN ($office_ids)";
                                        ...
                                        <button style="min-width: 15%; float: left;" type="button" id="addRowBtn" class="btn btn-dark btn-sm sessions-add">إضافة جلسة</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <form method="post" class="shadow p-3 mt-4" id="clientEdit" action="">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <?php
                                // Prepare SQL statement to fetch offices for the logged-in admin
                                $sql = "SELECT office_id, office_name FROM offices WHERE admin_id = :admin_id";
                                $stmt = $conn->prepare($sql);
                                $stmt->bindParam(':admin_id', $admin_id, PDO::PARAM_INT);
                                $stmt->execute();
                                $resultOffice = $stmt->fetchAll(PDO::FETCH_ASSOC);

                                if (count($resultOffice) > 0) {
                                    foreach ($resultOffice as $rowOffice) {
                        ...
                        <div class="row">
                            ...
                            <label class="form-label">الدور</label>
                                ...
                                <option value="" disabled selected>اختر الدور</option>
                                    <?php
                                        // Prepare SQL statement to fetch office IDs for the logged-in admin
                                        $sql_offices = "SELECT office_id FROM offices WHERE admin_id = :admin_id";
                                        $stmt_offices = $conn->prepare($sql_offices);
                                        ...
                                        if (!empty($offices)) {
                                            ...
                                            // Prepare SQL statement to fetch roles for the fetched office IDs
