...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admins') {
    ...
    if (!empty($offices)) {
        ...
        $sql = "SELECT courts.*, offices.office_name FROM courts INNER JOIN offices ON courts.office_id = offices.office_id WHERE courts.office_id IN ($office_ids) OR public = 1";
        ...
        if (!empty($search)) {
            ...
            $sqlCount = "SELECT COUNT(*) FROM courts INNER JOIN offices ON courts.office_id = offices.office_id WHERE courts.office_id IN ($office_ids) AND (court_name LIKE ? OR offices.office_name LIKE ?)";
            ...
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $total_records = $conn->query("SELECT COUNT(*) FROM courts WHERE office_id IN ($office_ids)")->fetchColumn();
            $total_pages = ceil($total_records / $total_records_per_page);
        }
        ...
        echo "No offices found for this admin.";
    }
    ?>
                    ...
                    <div class="modal-body">
                        <form id="editFormCourts" method="post" action="req/court-edit.php">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="officeIdEdit" class="form-select" name="office_id">
                ...
                <form method="post" class="shadow p-3 mt-5 n-table" action="req/court-add.php" style="direction:rtl;">
                    ...
                    <div class="mb-3">
                        ...
                        <select id="office_id" class="form-select" name="office_id" required>
                            ...
                            <?php
                                $admin_id = $_SESSION['user_id']; // جلب معرف الآدمن من الجلسة
                                $sql = "SELECT `office_id`, `office_name` FROM offices WHERE admin_id = :admin_id";
                                $stmt = $conn->prepare($sql);
                                $stmt->bindParam(':admin_id', $admin_id, PDO::PARAM_INT);
                                $stmt->execute();
