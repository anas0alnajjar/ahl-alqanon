...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admins') {
    ...
    if (!empty($offices)) {
        ...
        // Base SQL query
        $sql = "SELECT * FROM `lawyer` WHERE office_id IN ($office_ids)";
        ...
        // Count total records
        if (empty($search)) {
            $total_records = $conn->query("SELECT COUNT(*) FROM lawyer WHERE office_id IN ($office_ids)")->fetchColumn();
        } else {
            $count_sql = "SELECT COUNT(*) FROM lawyer WHERE office_id IN ($office_ids) AND (lawyer.lawyer_name LIKE :searchParam OR lawyer.lawyer_email LIKE :searchParam OR lawyer.lawyer_phone LIKE :searchParam OR lawyer.lawyer_address LIKE :searchParam OR lawyer.lawyer_city LIKE :searchParam)";
        ...
        // Rest of your code to handle pagination and displaying data
        // ...

    } else {
        echo "لا توجد مكاتب مرتبطة بهذا المسؤول.";
    }
