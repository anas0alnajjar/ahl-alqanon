...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Helper') {
    ...
    if (!empty($OfficeId)) {
        // Base SQL query to fetch clients related to helper
        $sql = "
            SELECT DISTINCT cl.client_id, cl.first_name, cl.last_name, cl.address, cl.city, cl.email, cl.phone
            FROM clients cl
        ...
        $searchParam = "%$search%";
        $params = [':helper_id' => $user_id];

        if (!empty($search)) {
            $sql .= " AND (CONCAT(cl.first_name, ' ', cl.last_name) LIKE :search OR cl.address LIKE :search OR cl.city LIKE :search OR cl.email LIKE :search OR cl.phone LIKE :search)";
        ...
        if (empty($search)) {
            ...
            $total_records_stmt->bindParam(':helper_id', $user_id, PDO::PARAM_INT);
            ...
            $count_sql = "
                ...
                JOIN cases ca ON cl.client_id = ca.client_id
                WHERE FIND_IN_SET(:helper_id, ca.helper_name)
                AND (CONCAT(cl.first_name, ' ', cl.last_name) LIKE :search OR cl.address LIKE :search OR cl.city LIKE :search OR cl.email LIKE :search OR cl.phone LIKE :search)
            ";
...
<!-- End -->
    ...
    <div class="container mt-5" style="direction: rtl;">
            <div class="btn-group mb-3" style="direction:ltr;">
                ...
                <a href="client-add.php" class="btn btn-dark clients-add">إضافة موكل للمحامي</a>
                <?php endif; ?>
             </div> 
    ...
    <?php } else { ?>
        <div class="alert alert-info w-450 d-flex align-items-center justify-content-between mt-3 mx-5" role="alert">
            ...
            <?php if ($pages['clients']['add']) : ?>
            <a href="client-add.php" class="btn btn-primary">أضف موكل جديد للمحامي</a>
            <?php endif; ?>
        </div>
    <?php } ?>
