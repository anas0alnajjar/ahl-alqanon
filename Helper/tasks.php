...
if (isset($_SESSION['user_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Helper') {
        ...
        $sql = "SELECT todos.*, 
                ...
                WHERE FIND_IN_SET(:helper_id, todos.helper_id)";
        ...
        $count_stmt->bindParam(':helper_id', $user_id, PDO::PARAM_INT);
        if (!empty($search)) {
...
<div class="container mt-5">
                                ...
                                <div class="todo-item border p-3 mb-2 rounded" style="direction:rtl;text-align:right;">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div>
                                            ...
                                            <?php } ?>
                                            <span class="task-title <?php echo $todo['read_by_helper'] ? 'checked' : ''; ?> mb-0">
    ...
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item <?php echo ($page_number <= 1) ? 'disabled' : ''; ?>">
            <a class="page-link" href="?page_number=<?php echo max(1, $page_number - 1); ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">السابق</a>
        </li>

        <!-- First Page -->
        <?php if ($page_number > 3): ?>
            <li class="page-item">
                <a class="page-link" href="?page_number=1&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">1</a>
            </li>
        ...
        for ($i = $start; $i <= $end; $i++): ?>
            <li class="page-item <?php echo ($page_number == $i) ? 'active' : ''; ?>">
                <a class="page-link" href="?page_number=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>"><?php echo $i; ?></a>
            </li>
        <?php endfor; ?>
        ...
        <?php if ($page_number < $total_pages - 2): ?>
            <li class="page-item disabled"><span class="page-link">...</span></li>
            <li class="page-item">
                <a class="page-link" href="?page_number=<?php echo $total_pages; ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>"><?php echo $total_pages; ?></a>
            </li>
        <?php endif; ?>

        <!-- Next Page -->
        <li class="page-item <?php echo ($page_number >= $total_pages) ? 'disabled' : ''; ?>">
            <a class="page-link" href="?page_number=<?php echo min($total_pages, $page_number + 1); ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">التالي</a>
        </li>
    </ul>
</nav>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row mb-3">
                    ...
                    <input type="hidden" name="helper_id" id="helper_name" value="<?=$user_id?>">
            ...
            <form id="editTaskForm" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <input type="hidden" name="helper_id" id="edit_helper_name" value="<?=$user_id?>">
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-6">
