...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $variables = [
        ...
        'تاريخ الجلسة' => '{$dueDate}',
        'ساعة الجلسة' => '{$dueHour}',
        'اسم المحامي' => '{$lawyer_name}'
    ];
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM templates WHERE type_template = 1 AND office_id = ? AND for_whom = ?");
    ...
    // حفظ الرسالة الجديدة
    $stmt = $conn->prepare("INSERT INTO templates (message_text, type_template, office_id, for_whom) VALUES (?, 1, ?, ?)");
