...
if (isset($_SESSION['user_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Helper') {
        if (isset($_POST['content']) &&
            ...
            // استعداد الاستعلام
            $sql  = "INSERT INTO documents(title, content, client_id, lawyer_id, attachments, office_id, case_id) VALUES(?,?,?,?,?,?, ?)";
            $stmt = $conn->prepare($sql);

            // تنفيذ الاستعلام
            $stmt->execute([$document_title, $content, $client_name, $lawer_name, $attachments_name, $office_id, $case_id]);
