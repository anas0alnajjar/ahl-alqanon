...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    $sql = "SELECT costs_type.*, offices.office_name FROM costs_type INNER JOIN offices ON costs_type.office_id = offices.office_id";

    // Prepare statement with placeholders
    if (!empty($search)) {
        $sql .= " WHERE costs_type.type LIKE ? OR offices.office_name LIKE ?";
        $sql .= " ORDER BY costs_type.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
        $sqlCount = "SELECT COUNT(*) FROM costs_type INNER JOIN offices ON costs_type.office_id = offices.office_id WHERE costs_type.type LIKE ? OR offices.office_name LIKE ?";
        ...
        $sql .= " ORDER BY costs_type.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
        $stmt = $conn->prepare($sql);
        $total_records = $conn->query("SELECT COUNT(*) FROM costs_type")->fetchColumn();
        $total_pages = ceil($total_records / $total_records_per_page);
    }
...
<div style="direction:rtl;" class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editForm" method="post" action="req/types-edit.php">
          <div class="mb-3">
