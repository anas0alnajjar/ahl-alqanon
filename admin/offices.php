...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    $sqlCount = "SELECT COUNT(*) FROM offices";

    if (!empty($search)) {
        $sql .= " WHERE office_name LIKE :search";
        $sqlCount .= " WHERE office_name LIKE :search";
    }
    ...
    // Render the rest of your HTML/PHP code here
?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" class="shadow p-3 mt-5 form-w" action="req/office-add.php" enctype="multipart/form-data" style="direction:rtl;">
                    ...
                    <div class="mb-3">
                        <label for="admin_id" class="form-label">الآدمن </label>
                        <select id="admin_id" class="form-select" name="admin_id" required>
                            <option value="" disabled selected>اختر الآدمن</option>
                            <?php
                                ...
                                if ($result3->rowCount() > 0) {
                                    while ($row3 = $result3->fetch(PDO::FETCH_ASSOC)) {
                                        $id = $row3["admin_id"];
                                        $admin_name = $row3["fname"] . ' ' . $row3['lname'];
                                        echo "<option value='$id'>$admin_name</option>";
                                    }
                                }
                            ?>
                        </select>
                    ...
                    <div class="form-check custom-checkbox mb-3">
                        <input type="checkbox" class="form-check-input" id="default_office" name="default_office" value="1">
                        <label class="form-label" for="default_office">مكتب افتراضي</label>
                    </div>
                    <div class="mb-3">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="editModalLabel">تعديل المكتب</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="position: absolute;left: 5px;"></button>
      </div>
      <div class="modal-body">
        <form id="editForm" method="post" action="req/office-edit.php" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            <label for="admin_id" class="form-label">الآدمن</label>
            <select id="admin_id_edit" class="form-select" name="admin_id">
              <option value="" disabled selected>اختر الآدمن</option>
              <?php
                  $sql = "SELECT * FROM `admin`";
                  $result3 = $conn->query($sql);
          ...
          <div class="form-check custom-checkbox mb-3">
                        <input type="checkbox" class="form-check-input" id="default_office2" name="default_office" value="1">
                        <label class="form-label" for="default_office2">مكتب افتراضي</label>
                    </div>
          <div class="mb-3">
...
if (!empty($result)) {
    ...
    <div class="container mt-5" style="direction: rtl;">
        <div class="btn-group mb-3" style="direction:ltr;">
            ...
            <button class="btn btn-dark" id="office-add"> اضافة مكتب جديد</button>
        </div>
                            ...
                            foreach ($result as $row): ?>
                                <tr>
                                    ...
                                    <td style="text-wrap:nowrap;">
                                        ...
                                        <a href="req/office-delete.php?id=<?=$row['office_id']?>" class="btn btn-danger btn-sm m-auto delete-button" data-id="<?=$row['office_id']?>">حذف</a>
                                        <button data-id="<?=$row['office_id']?>" class="btn btn-warning btn-sm m-auto offices-write">تعديل</button>
                                    </td>
                                    <td style="display:none;" data-default-office="<?=$row['default_office']?>"><?= $row['default_office'] ? 'نعم' : 'لا' ?></td>
                                </tr>
                                <?php $i++; ?>
                            <?php endforeach; ?>
                        </tbody>
...
<script>
    ...
    var editButtons = document.querySelectorAll('.offices-write');
    editButtons.forEach(function(button) {
        button.addEventListener("click", function() {
            ...
            var default_office = row.querySelector("td[data-default-office]").getAttribute("data-default-office").trim() === '1';
            ...
            document.getElementById("default_office2").checked = default_office;
