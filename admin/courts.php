...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    $sql = "SELECT courts.*, offices.office_name FROM courts INNER JOIN offices ON courts.office_id = offices.office_id";

    // Prepare statement with placeholders
    if (!empty($search)) {
        $sql .= " WHERE court_name LIKE ? OR offices.office_name LIKE ?";
        $sql .= " ORDER BY courts.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
        $sqlCount = "SELECT COUNT(*) FROM courts INNER JOIN offices ON courts.office_id = offices.office_id WHERE court_name LIKE ? OR offices.office_name LIKE ?";
        ...
        $sql .= " ORDER BY courts.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
        $stmt = $conn->prepare($sql);
        $total_records = $conn->query("SELECT COUNT(*) FROM courts")->fetchColumn();
        $total_pages = ceil($total_records / $total_records_per_page);
    }
...
<div style="direction:rtl;" class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editForm" method="post" action="req/court-edit.php">
          <div class="mb-3">
    ...
    <a href="home.php" class="btn btn-light">الرئيسية</a>
        <button class="btn btn-dark" id="disability-add"> اضافة محكمة جديدة</button>
    </div>    
...
<nav aria-label="Page navigation example">
    ...
    <?php } else { ?>
        ...
        <button href="" class="btn btn-dark mx-2" id="disability-add">اضافة محكمة جديدة</button>
        <span class="flex-grow-1 ">لا يوجد محاكم حتى الآن</span>
