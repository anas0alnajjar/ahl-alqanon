...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    if (!empty($search)) {
        $sql .= " WHERE helper_name LIKE ? OR username LIKE ? 
    ...
    // Count total records
    $total_records = $conn->query("SELECT COUNT(*) FROM helpers")->fetchColumn();
...
<nav aria-label="Page navigation example">
    ...
    <?php } else { ?>
        <div class="alert alert-info d-flex align-items-center mt-3" role="alert" style="max-width: 400px; margin-left: auto;margin-right: 2rem;justify-content: space-around;">
            ...
            <button type="button" style="text-align: right;" class="btn btn-dark" id="addHelper">إضافة إداري</button>
        </div>
    <?php } ?>
                    ...
                    <div class="row">
                            ...
                            <select id="lawyer_id555" class="form-select col-md-12" name="lawyer_id555">
                                ...
                                if ($result54->rowCount() > 0) {
                                    while ($row = $result54->fetch(PDO::FETCH_ASSOC)) {
                                        ...
                                        echo "<option value='$lawyer_id54'>$lawyer_name54</option>";
                                    }
                                }
                                ?>
                            </select>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select id="role_idHelper" class="form-select" name="role_id" required>
                                ...
                                <?php
                                $sql = "SELECT `power_id`, `role`, `default_role_helper` FROM powers";
                                $result2 = $conn->query($sql);
                                if ($result2->rowCount() > 0) {
                                    while ($row2 = $result2->fetch(PDO::FETCH_ASSOC)) {
                                        $id = $row2["power_id"];
                                        $role = $row2["role"];
                                        $default_role_helper = $row2["default_role_helper"];
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="tel" class="form-control" id="phoneHelper" name="phone" style="direction:ltr;" required>
                        </div>
