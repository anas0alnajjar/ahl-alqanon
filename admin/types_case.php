...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    $sql = "SELECT types_of_cases.*, offices.office_name FROM types_of_cases INNER JOIN offices ON types_of_cases.office_id = offices.office_id";
    $sqlCount = "SELECT COUNT(*) FROM types_of_cases INNER JOIN offices ON types_of_cases.office_id = offices.office_id";

    // Prepare statement with placeholders for search
    if (!empty($search)) {
        $sql .= " WHERE type_case LIKE ? OR offices.office_name LIKE ?";
        $sqlCount .= " WHERE type_case LIKE ? OR offices.office_name LIKE ?";
    }

    // Append pagination to SQL query
    $sql .= " ORDER BY types_of_cases.id DESC LIMIT " . (int)$offset . ", " . (int)$total_records_per_page;
...
<div style="direction:rtl;" class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editForm" method="post" action="req/types-cases-edit.php">
          <div class="mb-3">
