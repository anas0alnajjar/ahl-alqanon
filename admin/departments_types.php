...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    $sql = "SELECT departments.*, offices.office_name FROM departments LEFT JOIN offices ON departments.office_id = offices.office_id";

    // Prepare statement with placeholders
    if (!empty($search)) {
        $sql .= " WHERE departments.type LIKE ? OR offices.office_name LIKE ?";
        $sql .= " ORDER BY departments.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
        $sqlCount = "SELECT COUNT(*) FROM departments INNER JOIN offices ON departments.office_id = offices.office_id WHERE departments.type LIKE ? OR offices.office_name LIKE ?";
        ...
        $sql .= " ORDER BY departments.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
        $stmt = $conn->prepare($sql);
        $total_records = $conn->query("SELECT COUNT(*) FROM departments")->fetchColumn();
        $total_pages = ceil($total_records / $total_records_per_page);
    }
...
<div style="direction:rtl;" class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editForm" method="post" action="req/department-edit.php">
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <select id="officeIdEdit" class="form-select" name="office_id" required>
             ...
             <div class="mb-3">
            <input type="checkbox" name="public" id="publicEdit">
            <label for="publicEdit">عام</label>
          </div>
...
<table class="table table-responsive mt-3 n-table">
    <thead>
        <tr>
            ...
            <th scope="col" style="font-weight:700;"> الدائرة  </th>
        ...
        foreach ($result as $row): ?>
            <tr>
                ...
                <a href="req/departments-delete.php?id=<?=$row['id']?>" class="btn btn-danger m-auto btn-sm delete-button" data-id="<?=$row['id']?>">حذف</a>
                    <button data-id="<?=$row['id']?>" data-office-id="<?=$row['office_id']?>" data-public="<?=$row['public']?>" class="btn btn-warning btn-sm m-auto">تعديل</button>
                </td>
            </tr>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    editButtons.forEach(function(button) {
        button.addEventListener("click", function() {
            ...
            var publicStatus = this.getAttribute("data-public") === "1"; // Assume public status is stored in a data attribute
            ...
            officeSelect.value = officeId; // Set the office select to the current office_id
             ...
             var publicCheckbox = document.getElementById("publicEdit");
            publicCheckbox.checked = publicStatus; // Set the public checkbox to the current status
