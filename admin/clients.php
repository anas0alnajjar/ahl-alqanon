...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    if (!empty($search)) {
        $sql .= " WHERE CONCAT(clients.first_name, ' ', clients.last_name) LIKE ? OR clients.address LIKE ? OR clients.city LIKE ? OR clients.email LIKE ? OR clients.phone LIKE ?";
    ...
    // Count total records
    if (empty($search)) {
        $total_records = $conn->query("SELECT COUNT(*) FROM clients")->fetchColumn();
    } else {
        $count_sql = "SELECT COUNT(*) FROM clients WHERE CONCAT(clients.first_name, ' ', clients.last_name) LIKE ? OR clients.address LIKE ? OR clients.city LIKE ? OR clients.email LIKE ? OR clients.phone LIKE ?";
...
<!-- End -->
    ...
    <div class="container mt-5" style="direction: rtl;">
            <div class="btn-group mb-3" style="direction:ltr;">
                ...
                <a href="client-add.php" class="btn btn-dark">إضافة عميل</a>
             </div> 
        <form action="clients.php" class="mt-3 align-center" method="GET">
            <div class="input-group mb-3 text-center" style="max-width: 100%;min-width:80%; direction: ltr;">
                <input type="text" class="form-control" name="search" placeholder="ابحث عن عميل هنا..." value="<?php echo htmlentities($search); ?>">
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body" style="overflow:hidden;">
                                ...
                                <a style="text-wrap:nowrap;" href="client-edit.php?client_id=<?=$row['client_id']?>" class="btn btn-warning btn-sm">تعديل</a>
                                <a style="text-wrap:nowrap;" href="client-delete.php?client_id=<?=$row['client_id']?>" class="btn btn-danger btn-sm delete-button" data-id="<?=$row['client_id']?>">حذف</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
    ...
    <?php } else { ?>
        <div class="alert alert-info w-450 d-flex align-items-center justify-content-between mt-3 mx-5" role="alert">
            <span>لا يوجد عملاء حتى الآن!</span>
            <a href="client-add.php" class="btn btn-primary">أضف عميل جديد</a>
        </div>
    <?php } ?>
