...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    if (empty($search)) {
        ...
        $sql .= " ORDER BY lawyer_id DESC ";}

    // Prepare statement with placeholders
    if (!empty($search)) {
        $sql .= " WHERE lawyer.lawyer_name LIKE ? OR lawyer.lawyer_email LIKE ? OR lawyer.lawyer_phone LIKE ? OR lawyer.lawyer_address LIKE ? OR 
                    lawyer.lawyer_city LIKE ?
    ...
    if (empty($search)) {
        ...
        $sql = "SELECT COUNT(*) FROM lawyer WHERE lawyer.lawyer_name LIKE ? OR lawyer.lawyer_email LIKE ? OR lawyer.lawyer_phone LIKE ? OR lawyer.lawyer_address LIKE ? OR lawyer.lawyer_city LIKE ?";
        $stmt = $conn->prepare($sql);
        $searchParam = "%$search%";
        $stmt->bindParam(1, $searchParam, PDO::PARAM_STR);
        $stmt->bindParam(2, $searchParam, PDO::PARAM_STR);
