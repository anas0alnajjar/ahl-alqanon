...
<?php
try {
    // تحديد user_id المناسب هنا
    $request_id = 1; // تأكد من تغيير هذه القيمة حسب السياق

    // جلب القيم الحالية من قاعدة البيانات
    $stmt2 = $conn->prepare("SELECT automatic_acceptance, `days` FROM requests WHERE id = :request_id");
    ...
    // التحقق من القيم
    if (!$row2) {
        $row2 = ['automatic_acceptance' => 0, 'days' => 0];
    }
...
<body>
    ...
    <h2 class="text-center mb-4">طلبات الانضمام</h2>
        <div class="d-flex justify-content-between mb-3" style="direction:ltr;">
            <div>
                <input type="number" id="days" name="days" class="form-control-sm ms-2" value="<?php echo htmlentities(isset($row2['days']) ? $row2['days'] : ''); ?>" min="0" placeholder="عدد الأيام">
                <div class="form-check form-check-inline ms-2">
                    <input class="form-check-input" type="checkbox" id="automatic_acceptance" name="automatic_acceptance" <?php echo isset($row2['automatic_acceptance']) && $row2['automatic_acceptance'] ? 'checked' : ''; ?>>
                    <label class="form-check-label" for="automatic_acceptance">قبول الطلبات بشكل أتوماتيكي</label>
                </div>
            </div>
            <div class="btn-group">
        ...
        <div class="form-container">
            <form style="min-width: 100%;" action="requests.php" method="GET" class="d-flex justify-content-center">
                <input style="direction:rtl;" type="text" name="search" class="form-control me-2" placeholder="البحث..." value="<?php echo htmlentities(isset($search) ? $search : ''); ?>">
...
<script>
    $(document).ready(function() {
        ...
        // تحديث التشيك بوكس
        $('#automatic_acceptance').change(function() {
            var automatic_acceptance = $(this).is(':checked') ? 1 : 0;
            $.ajax({
                url: 'req/update_requests.php',
                type: 'POST',
                data: {
                    field: 'automatic_acceptance',
                    value: automatic_acceptance
                },
