...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Admin') {
        ...
        function getClients($conn) {
            $sql = "SELECT client_id, first_name, last_name FROM clients";
        ...
        LEFT JOIN lawyer ON todos.lawyer_id = lawyer.lawyer_id
        ...
        if (!empty($search)) {
            $sql .= " AND (todos.title LIKE :search 
                    ...
                    OR lawyer.lawyer_name LIKE :search
                    OR helpers.helper_name LIKE :search)";
        }

        if (!empty($clientFilter)) {
            $sql .= " AND todos.client_id = :clientFilter";
        }

        if (!empty($lawyerFilter)) {
            $sql .= " AND todos.lawyer_id = :lawyerFilter";
        }
        ...
        if (!empty($clientFilter)) {
            $stmt->bindParam(':clientFilter', $clientFilter, PDO::PARAM_INT);
        }
        if (!empty($lawyerFilter)) {
            $stmt->bindParam(':lawyerFilter', $lawyerFilter, PDO::PARAM_INT);
        }
        ...
        if (!empty($clientFilter)) {
            $count_sql .= " AND todos.client_id = :clientFilter";
        }
        if (!empty($lawyerFilter)) {
            $count_sql .= " AND todos.lawyer_id = :lawyerFilter";
        }
        if ($dateFilter == 'today') {
        ...
        if (!empty($clientFilter)) {
            $count_stmt->bindParam(':clientFilter', $clientFilter, PDO::PARAM_INT);
        }
        if (!empty($lawyerFilter)) {
            $count_stmt->bindParam(':lawyerFilter', $lawyerFilter, PDO::PARAM_INT);
        }
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                <select id="dateFilter" name="dateFilter" class="form-control">
                                    <option value="all" <?= ($dateFilter == 'all') ? 'selected' : '' ?>>كل الاشعارات</option>
                            ...
                            <div class="col-md-4">
                                <select id="clientFilter" name="clientFilter" class="form-control">
                            ...
                            <div class="col-md-4">
                                <select id="lawyerFilter" name="lawyerFilter" class="form-control">
                                    <option value="">كل المحامين</option>
                                    <?php foreach ($lawyers as $lawyer): ?>
                                        <option value="<?= htmlspecialchars($lawyer['lawyer_id']) ?>" <?= ($lawyerFilter == $lawyer['lawyer_id']) ? 'selected' : '' ?>><?= htmlspecialchars($lawyer['lawyer_name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                                ...
                                <div class="todo-item border p-3 mb-2 rounded" style="direction:rtl;text-align:right;">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div>
                                            ...
                                            <?php } ?>
                                            <span class="task-title <?php echo $todo['checked'] ? 'checked' : ''; ?> mb-0">
                                    ...
                                    <div class="text-muted small">
                                        ...
                                        <div>المحامي: <?php echo $todo['lawyer_name'] ?></div>
                                        <?php if (!empty($todo['client_name'])): ?>
                                            <div>الموكل: <?php echo $todo['client_name']; ?></div>
                                        <?php endif; ?>
                                        <?php if (!empty($todo['helper_name'])): ?>
                                            <div>الإداري: <?php echo $todo['helper_name']; ?></div>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <div class="text-muted small mt-2 d-flex justify-content-start align-items-center">
                                        <?php if($todo['read_by_admin']) { echo '<div class="me-3"><i class="fas fa-eye text-success"></i> تم قراءتها من قبل الآدمن</div>'; } ?>
                                        <?php if($todo['read_by_manager']) { echo '<div class="me-3"><i class="fas fa-eye text-success"></i> تم قراءتها من قبل المدير</div>'; } ?>
                                        <?php if($todo['read_by_lawyer']) { echo '<div class="me-3"><i class="fas fa-eye text-primary"></i> تم قراءتها من قبل المحامي</div>'; } ?>
                                        <?php if($todo['read_by_client']) { echo '<div class="me-3"><i class="fas fa-eye text-warning"></i> تم قراءتها من قبل الموكل</div>'; } ?>
                                        <?php if($todo['read_by_helper']) { echo '<div class="me-3"><i class="fas fa-eye text-info"></i> تم قراءتها من قبل الإداري</div>'; } ?>
                                    </div>
                                </div>
                            <?php } ?>
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination justify-content-center">
                                ...
                                <li class="page-item <?php echo ($page_number <= 1) ? 'disabled' : ''; ?>">
                                    <a class="page-link" href="?page_number=<?php echo max(1, $page_number - 1); ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&lawyerFilter=<?php echo urlencode($lawyerFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">السابق</a>
                                </li>

                                <!-- First Page -->
                                <?php if ($page_number > 3): ?>
                                    <li class="page-item">
                                        <a class="page-link" href="?page_number=1&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&lawyerFilter=<?php echo urlencode($lawyerFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">1</a>
                                    </li>
                                ...
                                for ($i = $start; $i <= $end; $i++): ?>
                                    <li class="page-item <?php echo ($page_number == $i) ? 'active' : ''; ?>">
                                        <a class="page-link" href="?page_number=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&lawyerFilter=<?php echo urlencode($lawyerFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>"><?php echo $i; ?></a>
                                    </li>
                                <?php endfor; ?>
                                ...
                                <?php if ($page_number < $total_pages - 2): ?>
                                    <li class="page-item disabled"><span class="page-link">...</span></li>
                                    <li class="page-item">
                                        <a class="page-link" href="?page_number=<?php echo $total_pages; ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&lawyerFilter=<?php echo urlencode($lawyerFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>"><?php echo $total_pages; ?></a>
                                    </li>
                                <?php endif; ?>

                                <!-- Next Page -->
                                <li class="page-item <?php echo ($page_number >= $total_pages) ? 'disabled' : ''; ?>">
                                    <a class="page-link" href="?page_number=<?php echo min($total_pages, $page_number + 1); ?>&search=<?php echo urlencode($search); ?>&clientFilter=<?php echo urlencode($clientFilter); ?>&lawyerFilter=<?php echo urlencode($lawyerFilter); ?>&dateFilter=<?php echo urlencode($dateFilter); ?>">التالي</a>
                                </li>
                            </ul>
                        </nav>
    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label for="lawers" class="form-label">اختر محامي</label>
                        <select id="lawers" class="form-select" name="lawyer_id" required>
                            <option value="" selected disabled>اختر محامي...</option>
                            <?php
                            $sql_lawyers = "SELECT DISTINCT lawyer_id, lawyer_name FROM lawyer ORDER BY lawyer_id;";
                            $result_lawyers = $conn->query($sql_lawyers);
                            if ($result_lawyers->rowCount() > 0) {
                                while ($row_lawyer = $result_lawyers->fetch(PDO::FETCH_ASSOC)) {
                                    $lawyer_id = $row_lawyer["lawyer_id"];
                    ...
                    <div class="col-md-6">
                        <label for="helper_name" class="form-label">اختر إداري</label>
                        <select id="helper_name" class="form-select" name="helper_id">
                            <option value="" selected disabled>اختر إداري...</option>
                            <?php
                            $sql_helpers = "SELECT id, helper_name FROM helpers ORDER BY id;";
                            $result_helpers = $conn->query($sql_helpers);
                            if ($result_helpers->rowCount() > 0) {
                                while ($row_helper = $result_helpers->fetch(PDO::FETCH_ASSOC)) {
                                    $helper_id = $row_helper["id"];
                                    $helper_name = $row_helper["helper_name"];
                                    echo "<option value='$helper_id'>$helper_name</option>";
                                }
                            }
                            ?>
                        </select>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label for="client_name" class="form-label">اختر عميل</label>
                        <select id="client_name" class="form-select" name="client_id">
                            <option value="" selected disabled>اختر عميل...</option>
                            <?php
                            $sql_clients = "SELECT client_id, first_name, last_name FROM clients ORDER BY client_id;";
                            $result_clients = $conn->query($sql_clients);
                            if ($result_clients->rowCount() > 0) {
                                while ($row_client = $result_clients->fetch(PDO::FETCH_ASSOC)) {
                                    $client_id = $row_client["client_id"];
                                    $client_name = $row_client["first_name"] . ' ' . $row_client["last_name"];
        ...
        <div class="modal-content" style="max-height:80vh;overflow:auto;">
            ...
            <form id="editTaskForm" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="edit_lawers" class="form-label">اختر محامي</label>
                            <select id="edit_lawers" class="form-select" name="lawyer_id" required>
                                <option value="" selected disabled>اختر محامي...</option>
                                <?php
                                $sql_lawyers = "SELECT DISTINCT lawyer_id, lawyer_name FROM lawyer ORDER BY lawyer_id;";
                                $result_lawyers = $conn->query($sql_lawyers);
                        ...
                        <div class="col-md-6">
                            <label for="edit_helper_name" class="form-label">اختر إداري</label>
                            <select id="edit_helper_name" class="form-select" name="helper_id">
                                <option value="" selected disabled>اختر إداري...</option>
                                <?php
                                $sql_helpers = "SELECT id, helper_name FROM helpers ORDER BY id;";
                                $result_helpers = $conn->query($sql_helpers);
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="edit_client_name" class="form-label">اختر عميل</label>
                            <select id="edit_client_name" class="form-select" name="client_id">
                                <option value="" selected disabled>اختر عميل...</option>
                                <?php
                                $sql_clients = "SELECT client_id, first_name, last_name FROM clients ORDER BY client_id;";
                                $result_clients = $conn->query($sql_clients);
