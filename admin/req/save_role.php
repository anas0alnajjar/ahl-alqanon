    ...
    if ($_SESSION['role'] == 'Admin') {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                if ($default_role == 1) {
                    $check_sql = "SELECT role FROM powers WHERE default_role = 1";
                    $check_stmt = $conn->prepare($check_sql);
                    $check_stmt->execute();
                    $existing_default_role = $check_stmt->fetch(PDO::FETCH_ASSOC);

                    if ($existing_default_role) {
                        echo json_encode(['status' => 'error', 'message' => 'يوجد بالفعل دور افتراضي لطلبات الانضمام باسم: ' . $existing_default_role['role']]);
                        exit;
                    }
                }
                ...
                // إدراج الدور الجديد
                $stmt = $conn->prepare("INSERT INTO powers (`role`, office_id, default_role, lawyer_id, default_role_client, default_role_lawyer, default_role_helper, default_role_manager) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$role_name, $office_id, $default_role, $lawyer_ids_string, $default_role_client, $default_role_lawyer, $default_role_helper, $default_role_manager]);
