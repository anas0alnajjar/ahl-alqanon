...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    if (isset($_POST['types']) && !empty($_POST['types'])) {
        ...
        $public = isset($_POST['public']) ? 1 : 0; // تحقق من حالة التشيك بو


        // استعداد الاستعلام
        $sql  = "INSERT INTO types_of_cases (`type_case`, office_id, public) VALUES(?, ?, ?)";
        ...
        if ($stmt->execute([$types, $office_id, $public])) {
            ...
            header("Location: ../types_case.php?popsuccess=$sm");
            exit;
        } else {
            // في حالة عدم نجاح التنفيذ، قم بإعادة توجيه المستخدم مع رسالة خطأ
            $em = "حدث خطأ أثناء الحفظ";
            header("Location: ../types_case.php?poperror=$em");
            exit;
        }
    } else {
