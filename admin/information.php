...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Admin') {
    ...
    // استعلام لاحتساب عدد الموكلين
    $query_clients_count = "SELECT COUNT(*) AS clients_count FROM clients";

    // استعلام لاحتساب عدد الجلسات القادمة
    ...
    // استعلام لاحتساب عدد الجلسات الكلي
    ...
    // استعلام لاحتساب عدد القضايا الكلي
    $query_total_cases = "SELECT COUNT(*) AS total_cases_count FROM `cases`";

    $current_month = date('Y-m');

    // استعلام لاحتساب التكاليف المرتبطة بالشهر الحالي من كلا الجدولين
    $query_monthly_costs = "
    SELECT SUM(amount) AS monthly_costs
    FROM (
        SELECT amount FROM overhead_costs WHERE DATE_FORMAT(pay_date, '%Y-%m') = :current_month
        UNION ALL
        SELECT amount FROM expenses WHERE DATE_FORMAT(pay_date, '%Y-%m') = :current_month
    ) AS combined_costs
    ";
    

    // الدفعات المستحقة للشهر الحالي

    $query_monthly_payment = "SELECT SUM(amount_paid) AS monthly_paid FROM `payments` WHERE received != 1 OR received IS NOT NULL AND DATE_FORMAT(payment_date, '%Y-%m') = :current_month";
    $query_monthly_payment_total = "SELECT SUM(amount_paid) AS monthly_paid FROM `payments` WHERE DATE_FORMAT(payment_date, '%Y-%m') = :current_month";
