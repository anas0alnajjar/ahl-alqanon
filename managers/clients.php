...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Managers') {
    ...
    if (!empty($OfficeId)) {
        ...
        // Base SQL query
        $sql = "SELECT * FROM clients WHERE office_id IN ($OfficeId)";
        $searchParam = "%$search%";
        $params = [];
        ...
        // Count total records
        if (empty($search)) {
            $total_records = $conn->query("SELECT COUNT(*) FROM clients WHERE office_id IN ($OfficeId)")->fetchColumn();
        } else {
            $count_sql = "SELECT COUNT(*) FROM clients WHERE office_id IN ($OfficeId) AND (CONCAT(clients.first_name, ' ', clients.last_name) LIKE ? OR clients.address LIKE ? OR clients.city LIKE ? OR clients.email LIKE ? OR clients.phone LIKE ?)";
