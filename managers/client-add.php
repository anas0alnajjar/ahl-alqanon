...
<body>
        ...
        <form method="post" class="shadow p-3 form-w" action="req/client-add.php" id="clientForm">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    ?>
                    <input type="hidden" class="form-control" value="<?=$referrer?>" name="referrer">
                        

                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">اسم الأب</label>
                    <input type="text" class="form-control" value="<?=$father_name?>" name="father_name">
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">اسم الجد</label>
                    <input type="text" class="form-control" value="<?=$grandfather_name?>" name="grandfather_name">
                </div>
                <div class="col-md-6 mb-3">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select id="role_id" class="form-select" name="role_id">
                        ...
                        if (!empty($OfficeId)) {
                            ...
                            $sql = "SELECT `power_id`, `role`, `default_role_client` FROM powers WHERE office_id IN ($OfficeId) OR default_role_client = 1";
                            $result = $conn->query($sql);
                            if ($result->rowCount() > 0) {
                                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                    $id = $row["power_id"];
                                    $role = $row["role"];
                                    $selected = ($row['default_role_client'] == 1) ? "selected" : "";
                                ...
                                if (!empty($OfficeId)) {
                                    ...
                                    if ($stmt_lawyers->rowCount() > 0) {
                                        while ($row = $stmt_lawyers->fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            $selected = ($lawyer_id == $_GET['lawyer_id']) ? "selected" : "";
    ...
    <script>
        document.getElementById('referrerPage').value = document.referrer;
