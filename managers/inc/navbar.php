<?php
    ...
    $query = "
        ...
        AND (
            client_id IN (SELECT client_id FROM clients WHERE office_id = :officeId)
            OR lawyer_id IN (SELECT lawyer_id FROM lawyer WHERE office_id = :officeId)
            OR case_id IN (SELECT case_id FROM cases WHERE office_id = :officeId)
        )
    ";
            ...
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0" id="navLinks">
                ...
                <li class="nav-item dropdown">
                    ...
                    <?php foreach($navlangs as $navlang){?>
                            <li><a class="dropdown-item nav-linlk"  href="#" onclick="changeLanguage('<?=$navlang['id']?>')"><?=$navlang['name']?></a></li>
                      <?php  } ?>
