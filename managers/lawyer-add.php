                        ...
                        <div class="col-md-6">
                            ...
                            <select id="role_id" class="form-select" name="role_id">
                                ...
                                if (!empty($OfficeId)) {
                                    ...
                                    $sql = "SELECT `power_id`, `role`, `default_role_lawyer` FROM powers WHERE office_id IN ($OfficeId) OR default_role_lawyer = 1";
                                    $result = $conn->query($sql);
                                    if ($result->rowCount() > 0) {
                                        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                            $id = $row["power_id"];
                                            $role = $row["role"];
                                            $selected = ($row['default_role_lawyer'] == 1) ? "selected" : "";
