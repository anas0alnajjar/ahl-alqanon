                            ...
                            <?php
                                if (!empty($OfficeId)) {
                                    try {
                                        ...
                                        $stmt = $conn->prepare("SELECT * FROM lawyer WHERE office_id = :office_id ORDER BY lawyer_id");
                                        ...
                                        if ($stmt->rowCount() > 0) {
                                            ...
                                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                                $lawyer_id54 = htmlspecialchars($row["lawyer_id"], ENT_QUOTES, 'UTF-8');
                                                $lawyer_name54 = htmlspecialchars($row["lawyer_name"], ENT_QUOTES, 'UTF-8');
                                                $lawyer_email54 = htmlspecialchars($row["lawyer_email"], ENT_QUOTES, 'UTF-8');
                                        ...
                                        echo "<option value='' disabled>حدث خطأ أثناء جلب البيانات</option>";
                                    }
                                }
                                ?>
                            </select>
                            ...
                            <select id="role_idHelper" class="form-select" name="role_id" required>
                                ...
                                <?php
                                    if (!empty($OfficeId)) {
                                        try {
                                            // استخدام معاملات مكانية لتجنب حقن SQL
                                            $stmt = $conn->prepare("SELECT `power_id`, `role` FROM powers WHERE office_id = :office_id");
                                            $stmt->bindParam(':office_id', $OfficeId, PDO::PARAM_INT);
                                            $stmt->execute();
                                            ...
                                            if ($stmt->rowCount() > 0) {
                                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                                    $id = htmlspecialchars($row["power_id"], ENT_QUOTES, 'UTF-8');
