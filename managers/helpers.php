...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Managers') {
    ...
    if (!empty($OfficeId)) {
        ...
        // Base SQL query
        $sql = "SELECT * FROM helpers WHERE office_id IN ($OfficeId)";
        ...
        // Count total records
        $total_records_sql = "SELECT COUNT(*) FROM helpers WHERE office_id IN ($OfficeId)";
        if (!empty($search)) {
                    ...
                    <div class="row">
                            ...
                            <select id="lawyer_id555" class="form-select col-md-12" name="lawyer_id555">
                                <option value="">اختر المحامي...</option>
                                    ...
                                    if (!empty($OfficeId)) {
                                        ...
                                        $sql = "SELECT * FROM lawyer WHERE office_id IN ($OfficeId) ORDER BY lawyer_id";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->execute();
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select id="role_idHelper" class="form-select" name="role_id" required>
                                ...
                                <?php
                                    if (!empty($OfficeId)) {
                                        $sql = "SELECT `power_id`, `role`, `default_role_helper` FROM powers WHERE office_id IN ($OfficeId) OR default_role_helper = 1";
                                        $result = $conn->query($sql);
                                        if ($result->rowCount() > 0) {
                                            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                                $id = $row["power_id"];
                                                $role = $row["role"];
                                                $selected = ($row['default_role_helper'] == 1) ? "selected" : "";
