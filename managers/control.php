                            ...
                            <select id="lawer_name" class="form-select" name="lawer_name">
                                ...
                                <?php
                                   if ($office_id !== null) {
                                    ...
                                    $sqlLawyers = "SELECT lawyer_id, lawyer_name FROM lawyer WHERE office_id = ? ORDER BY lawyer_id;";
                                    $stmtLawyers = $conn->prepare($sqlLawyers);
                                    $stmtLawyers->execute([$office_id]);
                                    if ($stmtLawyers->rowCount() > 0) {
                                        while ($row = $stmtLawyers->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            <select id="client_name" class="form-select" name="client_name">
                                ...
                                <?php
                                    if ($office_id !== null) {
                                        ...
                                        $sqlClients = "SELECT client_id, first_name, last_name FROM clients WHERE office_id = ? ORDER BY client_id;";
                                        $stmtClients = $conn->prepare($sqlClients);
                                        $stmtClients->execute([$office_id]);
        ...
        <div class="p-6">
            <div class="flex flex-col lg:flex-row gap-6 mb-6">
                ...
                <div class="lg:flex-1 grid grid-cols-1 md:grid-cols-2 gap-6" style="">
                    ...
                    <!-- Card 2 -->
                    <div class="custom-card card-3" data-count="<?php echo $clients_count_result['clients_count']; ?>" data-text="عدد الموكلين">
                        <div class="flex justify-between mb-4">
                            <div>
                            <a href="clients.php"><i style="position: absolute;left: 10px;" class="fas fa-users text-gray-400 text-3xl pulse"></i></a>
                                <div class="text-xl font-semibold mb-1 text-count"></div>
                                <div class="text-sm font-medium text-description"></div>
                            </div>
                        </div>
                    ...
                    <!-- Card 4 -->
                    <div class="custom-card card-6" data-count="<?php echo $monthly_costs_result['monthly_costs']; ?>" data-text="المصروفات للشهر الحالي">
                        <div class="flex justify-between mb-4">
                            <div>
                                <i style="position: absolute;left: 10px;" class="fas fa-money-bill-alt text-gray-400 text-3xl pulse"></i> 
                                <div class="text-xl font-semibold mb-1 text-count"></div>
                        ...
                        <a href="genral_expenses.php" class="text-white font-medium text-sm hover:text-gray-200">صفحة المصروفات</a>
                    </div>
                  ...
                  <div class="custom-card card-5" data-count="<?php if (isset($monthly_payment_result['monthly_paid']) && $monthly_payment_result['monthly_paid'] > 0) { echo $monthly_payment_result['monthly_paid']; } ?>" data-text="الدفعات الغير مستلمة لهذا الشهر">
                    ...
                    <!-- Card 6 -->
                    <div class="custom-card card-5" data-count="<?php echo ($monthly_payment_result_total['monthly_paid'] - $monthly_costs_result['monthly_costs']); ?>" data-text="الأرباح/العجز للشهر الحالي">
                        <div class="flex justify-between mb-4">
                            <div>
                                <i style="position: absolute;left: 10px;" class="fas fa-plus-circle text-gray-400 text-3xl pulse"></i> 
                                <div class="text-xl font-semibold mb-1 text-count"></div>
                                <div class="text-sm font-medium text-description"></div>
                            </div>
                        </div>
                ...
                <div class="bg-white border border-gray-100 shadow-md p-6 rounded-md h-full">
                    <div class="w-full h-full">
                        ...
                        <select class="mb-2" name="" id="lawyers-events">
                ...
                <div class="bg-white border border-gray-100 shadow-md shadow-black/5 p-6 rounded-md lg:col-span-2 card">
                    ...
                    <div id="chart-container">
                        <select id="office-select">
                            <option value="" >قارن التكاليف بالمصاريف</option>
                        ...
                        <table class="w-full min-w-[460px]" id="clientDate">
                            <thead>
                                <tr style='text-wrap:nowrap;'>
                                    <th class="text-[12px] uppercase tracking-wide font-medium text-gray-400 py-2 px-4 bg-gray-50 text-left rounded-tl-md rounded-bl-md">اسم الموكل</th>
                                    <th class="text-[12px] uppercase tracking-wide font-medium text-gray-400 py-2 px-4 bg-gray-50 text-left">عدد الجلسات القادمة</th>
