...
$(document).ready(function() {
    function fetch(start_date = null, end_date = null) {
        $.ajax({
                ...
                $('#records').DataTable({
                    ...
                    "columns": [
                        ...
                        { "data": "notes", "title": "ملاحظات" },
                        {
                            ...
                            "visible": pages.expenses.delete == 1 // عرض العمود فقط إذا كانت الصلاحية موجودة
                        }
                    ],
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    dataType: 'json', // التأكد من أن نوع البيانات المستقبلة JSON
                    success: function(response) {
                        console.log(response); // لعرض استجابة الخادم في وحدة التحكم
                        if (response.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'تم !',
                    ...
                    error: function(xhr, status, error) {
                        console.log(xhr.responseText); // لعرض محتوى الاستجابة عند حدوث خطأ
                        Swal.fire(
                            'خطأ!',
                            'حدث خطأ أثناء الحذف.',
                            'error'
