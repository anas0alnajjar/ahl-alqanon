        ...
        if (isset($_POST['title']) && isset($_POST['lawyer_id']) && !empty($_POST['lawyer_id'])) {
            ...
            if (empty($title)) {
                ...
                if ($res) {
                    // إرسال رسالة للمحامي
                    if (!empty($lawyer_id)) {
                        $lawyer_stmt = $conn->prepare("SELECT lawyer_phone, lawyer_name FROM lawyer WHERE lawyer_id = ?");
                        $lawyer_stmt->execute([$lawyer_id]);
                        $lawyer = $lawyer_stmt->fetch(PDO::FETCH_ASSOC);
                        if ($lawyer) {
                            $lawyer_message = "السيد " . $lawyer['lawyer_name'] . "،\n\nلديك رسالة جديدة من العميل: $title.\n\nأتمنى أن تكون بخير.\n\n";
                            sendWhatsAppMessage($lawyer['lawyer_phone'], $lawyer_message, $settings);
                        }
                    }
