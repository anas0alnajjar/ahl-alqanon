...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Client') {
    ...
    if ($user_id !== null) {
        ...
        $stmt_sessions->bindParam(':client_id', $user_id, PDO::PARAM_INT);
        ...
        // جلب الأحداث المرتبطة بالموكلين
        $sql_events = "SELECT event_id, client_id, event_name, event_start_date, event_end_date 
                       FROM events
                       WHERE client_id = :client_id";

        $stmt_events = $conn->prepare($sql_events);
        $stmt_events->bindParam(':client_id', $user_id, PDO::PARAM_INT);
        $stmt_events->execute();
