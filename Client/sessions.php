...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Client') {
    ...
    if (!empty($caseFilter)) {
        $stmt->bindValue(':caseFilter', $caseFilter, PDO::PARAM_INT);
    }
    ...
    if (!empty($caseFilter)) {
        $total_records_sql .= " AND c.case_id = :caseFilter";
    }

    $total_records_stmt = $conn->prepare($total_records_sql);
    $total_records_stmt->bindValue(':client_id', $user_id, PDO::PARAM_INT);
    ...
    if (!empty($caseFilter)) {
        $total_records_stmt->bindValue(':caseFilter', $caseFilter, PDO::PARAM_INT);
    }
...
<nav aria-label="Page navigation example">
    <ul class="pagination" style="direction: ltr;float:right;">
        ...
        <li class="page-item <?php echo ($page_number <= 1) ? 'disabled' : ''; ?>">
            <a class="page-link" href="?page_number=<?php echo ($page_number - 1); ?>&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&caseFilter=<?= htmlentities($caseFilter) ?>">Previous</a>
        </li>

        <!-- First Page -->
        <?php if ($page_number > 3): ?>
            <li class="page-item">
                <a class="page-link" href="?page_number=1&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&caseFilter=<?= htmlentities($caseFilter) ?>">1</a>
            </li>
        ...
        <?php for ($i = max(1, $page_number - 2); $i <= min($total_pages, $page_number + 2); $i++): ?>
            <li class="page-item <?php echo ($page_number == $i) ? 'active' : ''; ?>">
                <a class="page-link" href="?page_number=<?php echo $i; ?>&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&caseFilter=<?= htmlentities($caseFilter) ?>"><?php echo $i; ?></a>
            </li>
        <?php endfor; ?>
        ...
        <?php if ($page_number < $total_pages - 2): ?>
            ...
            <li class="page-item">
                <a class="page-link" href="?page_number=<?php echo $total_pages; ?>&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&caseFilter=<?= htmlentities($caseFilter) ?>"><?php echo $total_pages; ?></a>
            </li>
        <?php endif; ?>

        <!-- Next Page -->
        <li class="page-item <?php echo ($page_number >= $total_pages) ? 'disabled' : ''; ?>">
            <a class="page-link" href="?page_number=<?php echo ($page_number + 1); ?>&search=<?= htmlentities($search) ?>&dateFilter=<?= htmlentities($dateFilter) ?>&caseFilter=<?= htmlentities($caseFilter) ?>">Next</a>
        </li>
    </ul>
</nav>
        </div>
