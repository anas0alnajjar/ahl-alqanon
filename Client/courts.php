...
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] == 'Client') {
    ...
    if (!empty($OfficeId)) {
        ...
        $sql = "SELECT courts.*, offices.office_name FROM courts INNER JOIN offices ON courts.office_id = offices.office_id WHERE courts.office_id IN ($OfficeId)";

        // Prepare statement with placeholders
        if (!empty($search)) {
            $sql .= " AND (court_name LIKE ? OR offices.office_name LIKE ?)";
            $sql .= " ORDER BY courts.id DESC LIMIT " . (int)$offset . ', ' . (int)$total_records_per_page;
            $sqlCount = "SELECT COUNT(*) FROM courts INNER JOIN offices ON courts.office_id = offices.office_id WHERE courts.office_id IN ($OfficeId) AND (court_name LIKE ? OR offices.office_name LIKE ?)";
            ...
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $total_records = $conn->query("SELECT COUNT(*) FROM courts WHERE office_id IN ($OfficeId)")->fetchColumn();
            $total_pages = ceil($total_records / $total_records_per_page);
        }
    ...
    ?>
    <!-- Rest of your HTML and PHP code for displaying the courts -->
...
<body>
    ...
    <div class="container mt-5" style="direction: rtl;">
        <div class="btn-group" style="direction:ltr;">
            ...
            <?php if ($pages['courts']['add']) : ?>
                <button class="btn btn-dark" id="court-add">اضافة محكمة جديدة</button>
            <?php endif; ?>
        </div>    
        <form action="courts.php" class="mt-3 n-table" method="GET">
            ...
            <table class="table table-responsive mt-3 n-table">
                <thead>
                    <tr>
                        ...
                        <th scope="col" style="text-wrap:nowrap;text-align:center;font-weight:700;">المحكمة</th>
                        <?php if ($pages['courts']['write'] || $pages['courts']['delete']) : ?>
                ...
                <tbody>
                    ...
                    foreach ($result as $row): ?>
                        <tr>
                            <th scope="row"><?=$i?></th>
                            <td style="text-wrap:nowrap;text-align:center;"><?=$row['court_name']?></td>
                            <?php if ($pages['courts']['write'] || $pages['courts']['delete']) : ?>
                                <td style="text-wrap:nowrap;">
                                    <?php if ($pages['courts']['write']) : ?>
                                        <button data-id="<?=$row['id']?>" data-office-id="<?=$row['office_id']?>" class="btn btn-warning btn-sm m-auto">تعديل</button>
                                    <?php endif; ?>
                                    <?php if ($pages['courts']['delete']) : ?>
                                        <a href="req/court-delete.php?id=<?=$row['id']?>" class="btn btn-danger m-auto btn-sm delete-button" data-id="<?=$row['id']?>">حذف</a>
                                    <?php endif; ?>
                                </td>
                            <?php endif; ?>
                        </tr>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModalLabel">تعديل المحكمة</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="position: absolute;left: 5px;"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editFormCourts" method="post" action="req/court-edit.php">
                            <div class="mb-3">
                                <label for="typeInput" class="form-label">اسم المحكمة</label>
                                <input type="text" class="form-control" id="typeInput" name="type" required>
                            </div>
                            <input type="hidden" class="form-control" id="officeIdEdit" name="office_id" required>
    ...
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            <div class="modal-header" style="display:unset;">
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="position: absolute;left: 5px;"></button>
                <h5 style="float: right;" class="modal-title" id="editModalLabel">إضافة محكمة</h5>
            </div>
            <div class="modal-body">
                <form method="post" class="shadow p-3 mt-5 n-table" action="req/court-add.php" style="direction:rtl;">
                    ...
                    <!-- حقول النموذج... -->
                    <div class="mb-3">
                        <label for="types" class="form-label">اسم المحكمة</label>
                        <input type="text" class="form-control" value="" name="types" id="types" required>
                    </div>
                    <input type="hidden" class="form-control" value="<?=$OfficeId?>" name="office_id" id="office_id" required>
...
<script>
    document.getElementById('court-add').addEventListener('click', function() {
        $('#add_client_modal').modal('show');
